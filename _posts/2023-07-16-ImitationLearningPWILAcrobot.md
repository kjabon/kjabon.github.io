...
If you're unfamiliar with Acrobot, it's a classic control problem in which the "elbow" joint can be actuated, the "shoulder" joint swings freely, and the goal is (typically) swing to a vertical, upwards position, starting from a vertical downwards position.
...
For instance: suppose I was training a robotic hand to grasp and sort objects of various size, material, and shape, certainly I would be able to specify distance-based measures of reward: how far is the object from the target? But the reward for a "stable grasp" of an object is not so easily mathematically defined. 
