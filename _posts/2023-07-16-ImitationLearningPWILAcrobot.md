...
Not exactly the same as the PWIL controller, but pretty close! Keep in mind that this was trained with two sources of noise: the ILQR controller itself had its actions and observations perturbed, and PWIL additionally perturbs actions. This said, the performance is surprisingly good; training on more demonstrations for more timesteps would probably mimic the original controller even better.
