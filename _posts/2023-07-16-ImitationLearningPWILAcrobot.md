...
Now, our D4PG agent shoots straight to the target. Our PWIL-trained agent, as before, takes a few swings to build momentum before its final approach. Note, the PWIL training / loss function never changed (it doesn't see environment reward). This different result from the previous PWIL rollout is merely the effect of randomness during training, and acting during this particular rollout. If more care was taken to fix all RNG seeds through the entire process, we would expect the same result, to within numerical error.

So what exactly is the benefit of imitation learning? Looking purely at rewards, D4PG seemed to win out here. Just as well, it achieves the top position much more quickly. Well, considering the two gifs above, let's ask ourselves: which would we prefer to run on a real robot? If we're concerned with robotics (amongst many other applications), we're going to have to come out of the simulation into the real world at some point.
...
This last point is really the kicker for imitation learning. We know intuitively the "safer," "natural" trajectory. Supposing we have motion capture data from real gymnasts swinging to an upright position, we would be able to (barring the double-jointed hips humans would need to replicate the Acrobot).
