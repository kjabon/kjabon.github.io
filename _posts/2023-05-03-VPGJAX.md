...
where the ``behavior_log_probs`` are the same term as ``policy_log_probs``, only they were calculated earlier, with an older version of the policy network. Let's skip the ``rlax.clipped_surrogate_pg_loss()`` function for a moment.
