...
We've additionally wrapped this with ``hk.without_apply_rng()``. Our forward function's ``apply`` method may require randomness in general, e.g. if it uses dropout layers during training. In our case, an rng key is not required, so we use this convenience wrapper to avoid needing to pass in an extra parameter.
...
**Uhhh well is params the right object? I think I skipped that**
