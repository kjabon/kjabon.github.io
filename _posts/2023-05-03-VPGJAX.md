...
``FeedForwardNetwork`` simply wraps ``haiku.transform``’s outputs (``init`` and ``apply``) into an Acme container. 
...
All joking aside, you should already have an understanding of the full loop before we proceed (see [here](/blog/2023/VPG). I'm going to start from the top down, giving a high level explanation of the loop, and then dive into each component.
