...
### Advantage 
Let's compare R^f with our 1-step return. To keep it clear we'll set $\gamma=1$.

$$R^f_{t} = \sum_{t'=t+1}^{T}r_{t'} = r_{t+1}+r_{t+2}+\cdots + r_{T}$$

$$\approx G_{t:t+1} = r_{t+1} + V(s_{t+1})$$

If $$G$$ is supposed to approximate $$R^f$$, we see the value function $$V$$ encodes the future variation. What about our 0-step return, i.e. just the 
