...
We take the gradient of this with respect to the policy parameters $$\theta$$.
...
We evaluate the policy given the current policy parameters $$\theta_k$$:
...
How does our new friend work? I'll mostly defer to good explanations elsewhere, for example [here](https://github.com/HIPS/autograd/blob/master/docs/tutorial.md#whats-going-on-under-the-hood) and [here](https://en.wikipedia.org/wiki/Automatic_differentiation). The gist is that first, it keeps track of the computation graph of whatever functions you want to differentiate, from state $$s$$ and parameters $$\theta$$ all the way to $$\log\pi_\theta (a_t\vert s_t)\vert_{\theta_k}$$. Then, it uses this graph to multiply its way through the chain rule, resulting in the gradient(s) you want. And of course, this is done in parallel for all the time steps in your trajectory. 
