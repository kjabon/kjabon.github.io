...
And by sticking $$V_t$$ in the place of $$R_t^f$$, we *almost* have the complete policy gradient:
...
- The environment may have probabilistic transitions. For a given state and action, in general, the resulting transition to the next state is *also* sampled from a probability distribution, $$p(s_' \vert s, a)$$. We swept this one under the rug until now (and will continue to do so for the rest of this post, so don't worry).

Each variation at each time step ultimately leads to the variance of the future return $$R^f$$. The longer the trajectory, the more pronounced this effect will become. Therefore, we want to remove the effect of this future variation.

### Subtract a baseline

I'll start with the punchline: it turns out you can take the expression you want (the n-step return), and subtract another similar expression (a baseline) to reduce the variance of your approximation. The best choice for this is the value function, so taking the 1-step return as an example:
...
Now for the new part. This actually gives us a sample of the optimal policy gradient for **all** possible trajectories. What we would ideally like is the expectation over all trajectories:

$$\mathbb{E}\left[\hat{g}\right]=\mathbb{E}\left[\sum_{t=0}^{T}\nabla _{\theta}\log\pi_\theta (a_t\vert s_t)_{\theta _k} V_t \right]$$

$$=\oint_{\mathcal{T}} \sum_{t=0}^{T}\nabla _{\theta}\log\pi_\theta (a_t\vert s_t)_{\theta _k} V_t d\tau$$
...
$$\frac{1}{\left|\mathcal{D}\right|}\sum_{\tau\in\mathcal{D}}\sum_{t=0}^{T}\nabla _{\theta}\log\pi_\theta (a_t\vert s_t)_{\theta _k} V_t $$

Now, our goal is to reduce the variance of this policy gradient estimate. I'll replace $\sum_{t=0}^{T}\nabla _{\theta}\log\pi_\theta (a_t\vert s_t)_{\theta _k}$ with $\nabla_\theta P(\tau)$ for brevity, and revert to the integral form for the sake of argument: 


$$\oint_{\mathcal{T}} \nabla_\theta P(\tau) V_t d\tau$$
...
Since we're in the business of using "real-time" approximations, our **accurate** estimate of $$R^f$$ is the n-step return, i.e. something that looks like $$r_{t+1} + \gamma V(s_{t+1})$$. So we go from:
