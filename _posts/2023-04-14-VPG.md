...
Indeed we are. So perhaps we can combine the two approaches. What if I use the actual reward in the next time step, and add it to the value function's approximation of the future return from that point on? Aha, a slightly more accurate estimate!
...
$$G_{t:t+n} = R_{t+1} + \gamma R_{t+2}+ \cdots +\gamma^{n-1}R_{t+n}+\gamma V_{t+n-1}(S_{t+n})$$

Now the question becomes: what's the optimal tradeoff between accuracy and this learning delay? At this point you just need to experiment and see what works best for your problem, but I will tell you this: the answer is somewhere between 100% accuracy and zero delay (show the chart here). 
...
It also turns out you can do even better by doing a weighted average of all of these options: one, two, three, etc. real rewards, followed by an approximation, and also experimenting to see what the right weighting is. 

That's generalized advantage estimation (GAE) in a nutshell. The sum will of course be discounted as usual, so all but the first term have $$\gamma^t$$ applied to them. For more specifics in how these calculations are done, see the [paper](https://arxiv.org/abs/1707.06347).
