...
I'll start with the punchline: it turns out you can take the expression you want (the n-step return), and subtract another similar expression (a baseline) to reduce the variance of your approximation. The most common choice for this baseline is the value function, so taking the 1-step return as an example:
...
Now add and subtract the integral of another function we can evaluate, $$\phi$$ which approximates $$f$$:
...
Instead of estimating the integral on the left (our original problem), we estimate the first term on the right side. Since $$\phi\approx f$$, $$\text{Cov}(f,\phi)$$ is positive. If $$- 2 \text{Cov}(f,\phi)+\text{Var}(\phi)$$ is negative, $$\text{Var}(f-\phi) \lt \text{Var}(f)$$, so the new thing we need to estimate has lower variance. 

Are you beginning to notice the similarity to our problem? This is why subtracting the value function from the n-step return reduces the variance of the samples from all trajectories.
