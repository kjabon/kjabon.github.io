...
We configure our neural network output to be logits, and later follow this with a softmax to convert to probabilities. We then simply compute the log of these values: $$\log \left[ P_\theta (s)\right] $$. Since this is a vector, to get the log probability for the action of interest $$a$$, ($$a$$ will be an integer from 0 to n-1, n being the dimensionality of the actions), we simply grab the $$a$$'th value from this vector. In other words, we compute $$\log \left[ P_\theta (s)\right]_a $$.
