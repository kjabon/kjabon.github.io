...
Instead of estimating the integral on the left (our original problem), we estimate the first term on the right side. Now 

$$\text{Var}(f-\phi) = \text{Var}(f) - 2 \text{Cov}(f,\phi)+\text{Var}(\phi)$$

 Since $$\phi\approx f$$, $$\text{Cov}(f,\phi)$$ is positive. If $$- 2 \text{Cov}(f,\phi)+\text{Var}(\phi)$$ is negative, $$\text{Var}(f-\phi) \lt \text{Var}(f)$$, so the new thing we need to estimate has lower variance. 

Are you beginning to notice the similarity to our problem? Since it may not be obvious, I'll make it painfully explicit. Starting with
