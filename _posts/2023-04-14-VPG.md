...
We have a more accurate approximation with our n-step return $$G$$. We've left out one important piece, though. This isn't actually "Advantage" yet. The Advantage at its most *basic* level answers the question: how much better is the *actual* future return $$R^f$$ better than my estimate $$V(s)$$? In a sense, it's a measure of surprise. If taking an action $$a_t$$ given the state $$s_t$$ and transitioning to a state $$s_{t+1}$$ is more rewarding than we think that state is in general ($$V(s)$$), then surely we ought to adjust our policy to do that more often.

Since we're in the business of using approximations, our **accurate** (but still "real-time") estimate of $$R_f$$ is the n-step return, i.e. something that looks like $$r_{t+1} + \gamma V_t(S_{t+1})$$. So we go from:

$$A_t = R^f_t - V(s)$$

to

$$A_t = r_{t+1} + \gamma V_t(S_{t+1}) - V(s)$$
...
The n-step return handily deals with the bias problem in approximating the value function, but doesn't do much to help the variance. What is the source of the variance? Well, the **approximation** of the future return, of course. Since the value function $$V$$ at each time step is approximated by the same neural network, we should expect it to contribute similar "noise" or "variance" to the true return at two different states $$s$$, if we assume the change in state $$s_t - s_{t-1}$$ is small. If we subtract the "approximation" $$V(s_t)$$ from the "slightly more accurate approximation" $$r_t + V(s_t+1)$$, we remove some shared variance and are mostly left with the true value we care about: the advantage $$A_t$$. 
