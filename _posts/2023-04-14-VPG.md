...
description: The theory behind one of our introductoy algorithsm
...
Let's describe what happens in one iteration of the loop at a high level before diving in.

First, we collect many trajectories by allowing our current policy (represented by a neural network) to unfold in the environment, with some randomness built in, because we are sampling each action from a probability distribution (math) (and sometimes just greedily picking the "best" action as currently learned by the policy). Randomness may also occur as the result of the environment itself.

Now, if a policy gains an above-average amount of rewards in a particular trajectory, we will nudge the policy in the direction of the actions (given their respective states) which resulted in this trajectory. Conversely, if a trajectory comes with a lower reward, we nudge the policy away from taking those actions.

To estimate the return (the discounted sum of rewards) at each time step with low variance, we employ generalized advantage estimation. We train a neural network to represent the value function V_t, which is incorporated into our advantage term A_t.

The policy gradient is calculated as follows:


$$ \hat{g}_{k}=\frac{1}{\left| \mathcal{D}_k \right|}\sum_{\tau\in \mathcal{D}_k}\sum_{t=0}^{T}\nabla _{\theta}\log\pi_\theta (a_t|s_t)_{\theta _k} \hat{A}_t $$ <-actually, just use V_t
...
$$ \theta_{k+1}=\arg \min_\phi \frac{1}{\left| \mathcal{D}_k \right|T}\sum_{\tau\in \mathcal{D}_k}\sum_{t=0}^{T}\left(V_\phi(s_t)-\hat{R}_t  \right)^2 $$
