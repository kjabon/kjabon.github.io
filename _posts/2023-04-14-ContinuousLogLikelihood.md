...
$$f(\widebar{x}) = \frac{1}{\left(2\pi\right)^{\frac{k}{2}}\left| \widebar{\widebar{\Sigma}} \right|^{\frac{1}{2}}}\exp\left(-\frac{1}{2}\left(\widebar{x}-\widebar{\mu}\right)^T\widebar{\widebar{\Sigma}}^{-1}\left(\widebar{x}-\widebar{\mu}\right)\right)$$

where $$\Sigma$$ is the (diagonal) covariance matrix, and $$\mu$$ is the column vector of means for each dimension. x is the vector of positions along the probability density function whose likelihoods we're interested in.
