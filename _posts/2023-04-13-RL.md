...
When we transition to the new state based on our action, we obtain the reward associated with that transition from old to new state: $$r = R(s, s’)$$. The goal in the RL problem is defined as maximizing the “return,” or the sum of rewards $$r$$ in a trajectory $$\tau$$. The trajectory's return $$R(\tau)=\sum_{t=0}^{T}r_t$$. The actor maintains a policy $$\pi(a\vert s)$$. This function outputs the probability distribution of all possible actions $$a$$ at time $$t$$ in the trajectory $$\tau$$, given the state $$s_t$$ at time $$t$$. We can see the optimal policy $$\pi(a\vert s)$$ which achieves this goal immediately:
