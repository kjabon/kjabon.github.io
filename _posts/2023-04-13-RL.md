...
Since our goal is to be near to the target space, let’s define a reward function: $$R(s,s’) = |x-s’| + 4$$. 
 {% details Why + 4? %}
...
o is the location of our actor. Notice here the origin state doesn’t actually change the reward, because the initialization of the environment state doesn’t spit out a reward. 
<d-footnote>In RL, we don’t get a reward just for showing up, we get rewards for participation!</d-footnote>
