...
{% details “That looks funny,” I hear you say. %}
 The reward time step is offset by one for a few reasons. We don’t get any reward for the first time step $$t=0$$, i.e. for initializing the state $$s_0$$. Why? We haven’t taken any action yet. Also, the reward $$r$$ is associated with both state $$s$$ and $$s’$$, which live in separate time steps. We have to pick one formalism arbitrarily, so we assign the reward to the latter time step $$t+1$$, because the environment returns it at the same time as the next state $$s’$$. However, we group it with the former tuple in our trajectory $$\tau$$, because it’s associated with the action $$a_t$$ we took at that time step $$t$$. This tuple organization follows an intuitive loop from the actor’s point of view: “observe, act, get a reward,” then repeat.
...
\begin{array}{|l||c|c|} 
\hline\\ 
x^2 & \text{some text} & \text{more text}\\ 
\hline  \\ 
\hline \\ 
x^2+y^2=z^2 & & 0.1\\ 
\hline 
...
Since our goal is to be near to the target space, let’s define a reward function: R(s,s’) = |t-s’| + 4. 
 {% details Why +4? %}
