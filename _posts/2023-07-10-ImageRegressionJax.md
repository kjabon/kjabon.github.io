...
Here's the preprocessing code implementing RandAugment
<d-code block language="python">
#Each of these layers is implemented as a function in JAX in the same file, preprocessing.py

randomLayers = [
    flipX,
    translate,
    color_degen, # I think this one's ok, it's just making it greyer
    auto_contrast, #seems ok
    equalize, #seems ok
    random_contrast, #tested working, seems not jank
    brightness,
    cutout,
    rotate,
]
...
def randomAugmentJax(images, rng, policy, augmentations_per_image = 3, rate = 10/11,
    ...
    def apply_rand_layer(image, layer_select_key, layer_key):
        ...
        #TODO: convert to jax.lax.scan over indices
        return image

        #Going to need big branching "wheres" here for more layers...
